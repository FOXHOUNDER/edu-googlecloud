...
async function ceaseService(reqTimestamp, IdTessera, MotivoDichiarato) {
  return new Promise(function(resolve, reject) {
    const tipoRichiesta = 'Cessazione';
    const tableName = 'SERVIZIO_CESSAZIONE';
    const sqlStatement = `
    ...
    connection.bq.createQueryStream(options)
      ...
      .on('data', function(row) {
        ...
        if(row.httpResponseCode === 200) {
          const sqlStatement = `
            CALL ${connection.API_DATASET}.sp_CessazioneServizio(
              @v_uuid,
              @v_IdTessera,
              @v_MotivoDichiarato
            );
